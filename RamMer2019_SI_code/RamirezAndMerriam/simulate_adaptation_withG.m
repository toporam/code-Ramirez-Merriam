function [out]=simulate_adaptation_withG(v, X, j, cond1, cond2, a, b, sigma, granularity_level, model_type, reset_after, paradigm)
...
%
% Version 2.0 Is based on simulate_adaptation.m The current version
% includes a further input parameter used to control the level of
% granularity of the simulated data. 
%
% Version 1.0   8-June-2017     Coded by: Hunar
% Version 2.0   11-March 2019   Coded by: Ramírez (based on Ver. 1.0)

for vox = 1:v
    dt = X/180;   % stimulus dimension resolution
%     dt = X/360;   % stimulus dimension resolution
        ...
        for q=1:length(j);
            ...
            if(rem(q-1,reset_after)==0)
                ...
                elseif(model_type==5)
                    ...
                    % auxZero = max(temp) % fdo. Figure out where the NaNs come from
                    % auxNaN = temp/max(temp) % fdo. Figure out where the NaNs come from
                    % if isnan(auxNaN(:)), error('NaN found in rep(q,i,:)'),end % catch NaNs
                else
                    
                    error('Simplified parent function to only run for models 1,2 & 5') ; % This is, because I do not care for adaptation effects here
